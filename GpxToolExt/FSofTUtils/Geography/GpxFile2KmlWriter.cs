...
namespace FSofTUtils.Geography {
   /// <summary>
   /// schreibt eine GPX-Datei als KML/KMZ-Datei
   /// </summary>
   public class GpxFile2KmlWriter {
      ...
      */
      ...
      public void Write_gdal(string filename,
         ...
         int trackcount = gpx.TrackCount();

         int kmldatacount = trackcount;
         if (cola == null || colr == null || colg == null || colb == null || width == null)
            kmldatacount = 0;
         else {
            kmldatacount = Math.Min(kmldatacount, cola.Count);
            kmldatacount = Math.Min(kmldatacount, colr.Count);
            kmldatacount = Math.Min(kmldatacount, colg.Count);
            kmldatacount = Math.Min(kmldatacount, colb.Count);
            kmldatacount = Math.Min(kmldatacount, width.Count);
         }
         if (kmldatacount == 0) {
            cola = new List<uint>() { 255 };
            colr = new List<uint>() { 0 };
            colg = new List<uint>() { 0 };
            colb = new List<uint>() { 0 };
            width = new List<uint>() { 4 };
            kmldatacount = 1;
         }

         for (int t = 0; t < trackcount; t++) {
            GpxTrack track = null;
            if (gpx.InternalGpx == GpxFile.InternalGpxForm.OnlyNormalGpx) {   // GpxTrack "nachbauen"
               track = new GpxTrack();
               int tracksegcount = gpx.TrackSegmentCount(t);
               for (int s = 0; s < tracksegcount; s++) {
                  GpxTrackSegment segment = new GpxTrackSegment();
                  segment.Points.AddRange(gpx.GetTrackSegmentPointList(t, s));
                  track.InsertSegment(segment);
               }
               track.Name = gpx.GetTrackname(t);
            } else
               track = gpx.GetPoorTrack(t);
            insertTrack_gdal_gx(track,
                                cola[Math.Min(kmldatacount - 1, t)],
                                colr[Math.Min(kmldatacount - 1, t)],
                                colg[Math.Min(kmldatacount - 1, t)],
                                colb[Math.Min(kmldatacount - 1, t)],
                                width[Math.Min(kmldatacount - 1, t)]);
         }

         int waypointcount = gpx.WaypointCount();
      ...
      string codedText(string txt) {
         return GpxFile.Text2String(txt);
      }
      ...
      void insertTrack_gdal_gx(GpxTrack track,
         ...
         if (BaseElement.ValueIsValid(dtMin) ||
            ...
            sb.Append("<TimeSpan>");
            if (BaseElement.ValueIsValid(dtMin))
               sb.Append("<begin>" + GpxFile.DateTime2String(dtMin) + "</begin>");
            if (BaseElement.ValueIsValid(dtMax))
               sb.Append("<end>" + GpxFile.DateTime2String(dtMax) + "</end>");
            sb.Append("</TimeSpan>");
         }
         if (!string.IsNullOrEmpty(track.Description))
            sb.Append("<description>" + track.Description + "</description>");
            ...
            for (int p = 0; p < track.Segments[s].Points.Count; p++) {
               ...
               sb_when.Append("<when>" +
                              (BaseElement.ValueIsValid(pt.Time) ?
                                 GpxFile.DateTime2String(pt.Time) :
                                 "") +
                              "</when>");

               sb_coord.Append("<gx:coord>" +
                              GpxFile.Double2String(pt.Lon) + "," +
                              GpxFile.Double2String(pt.Lat) +
                              (BaseElement.ValueIsValid(pt.Elevation) ?
                                 "," + GpxFile.Double2String(pt.Elevation) :
                                 "") +
                              "</gx:coord>");
            }
      ...
      StringBuilder buildMultiGeometry(GpxTrack track) {
         ...
         for (int s = 0; s < track.Segments.Count; s++) {
            ...
            for (int p = 0; p < track.Segments[s].Points.Count; p++) {
               GpxTrackPoint pt = track.Segments[s].Points[p];
               sb.Append(" ");
               sb.Append(GpxFile.Double2String(pt.Lon));
               sb.Append(",");
               sb.Append(GpxFile.Double2String(pt.Lat));
               if (BaseElement.ValueIsValid(pt.Elevation)) {
                  sb.Append(",");
                  sb.Append(GpxFile.Double2String(pt.Elevation));
               }
            }
      ...
      void insertWaypoint_gdal(GpxWaypoint pt) {
         ...
         if (BaseElement.ValueIsValid(pt.Elevation))
            sb.Append("<SimpleData name=\"ele\">" + GpxFile.Double2String(pt.Elevation) + "</SimpleData>");
         if (BaseElement.ValueIsValid(pt.Time))
         ...
         sb.Append("</ExtendedData>");
         sb.Append("<Point><coordinates>" + GpxFile.Double2String(pt.Lon) + "," + GpxFile.Double2String(pt.Lat) + "</coordinates></Point>");
         sb.Append("</Placemark>");
         /*
            <Placemark>
