...
namespace GpxTool {
   class Program {
      ...
      */
      ...
      /// <param name="inputstream">falls kein Dateiname vorhanden, ist hier ein Stream nötig</param>
      /// <param name="idx"></param>
      /// <param name="cancel"></param>
      /// <param name="progress"></param>
      /// <returns></returns>
      static int TaskWorker4Read(string filename,
                                 Stream inputstream,
                                 int idx,
         ...
         gpxfiles[idx].Read(inputstream);
         if (inputstream != null) { // erst schließen nach vollständiger Verarbeitung
            inputstream.Close();
            inputstream.Dispose();
         }
      ...
      public static void Main(string[] args) {
         ...
         string title = "";
         Attribute attr;
         attr = Attribute.GetCustomAttribute(a, typeof(AssemblyProductAttribute));
         if (attr != null)
            title += (attr as AssemblyProductAttribute).Product + " ";
         attr = Attribute.GetCustomAttribute(a, typeof(AssemblyInformationalVersionAttribute));
         if (attr != null)
            title += (attr as AssemblyInformationalVersionAttribute).InformationalVersion + ", ";
         attr = Attribute.GetCustomAttribute(a, typeof(AssemblyCopyrightAttribute));
         if (attr != null)
            title += (attr as AssemblyCopyrightAttribute).Copyright;

         Console.Error.WriteLine(title);
         attr = Attribute.GetCustomAttribute(a, typeof(AssemblyDescriptionAttribute));
         if (attr != null)
            Console.Error.WriteLine((attr as AssemblyDescriptionAttribute).Description);
      ...
      /// <returns></returns>
      static GpxFileSpecial ReadAndMergeGpxfiles(List<string> Inputfiles,
                                                 string destfile,
                                                 bool showinfo,
                                                 bool simplify,
                                                 string gpxcreator) {
...
#if ANDROID
            using (MemoryStream stream = ReadAndroidFile(Inputfiles[0]))
               gpxfile.Read(stream);
#else
            gpxfile.Read(null);
#endif
...
#if ANDROID
               tq.StartTask(null as string,
                            ReadAndroidFile(Inputfiles[i]),
                            i,
                            TaskWorker4Read,
                            progress);
#else
               tq.StartTask(Inputfiles[i],
                            null as Stream,
                            i,
                            TaskWorker4Read,
                            progress);
      ...
      static void ProcessGpxfile(GpxFileSpecial gpxfile, Options opt) {
         ...
         if (opt.HeightOutputfile.Length > 0) {
            if (File.Exists(opt.HeightOutputfile) && !opt.OutputOverwrite)
               ...
               gpxfile.SaveHeight(null, outputstream);
               SaveAndroidFile(opt.HeightOutputfile, outputstream);
#else
               gpxfile.SaveHeight(opt.HeightOutputfile, null);
#endif
            }
         }
      }
      ...
      static void SaveGpxfile(GpxFileSpecial gpxfile,
         ...
         string destfileandroid = destfile;
         destfile = null;
#endif
      ...
      // Das Lesen und Schreiben unter Android fkt. nur über einen einfachen Stream. Dafür wird hier ein MemoryStream als "Zwischenobjekt" verwendet.

      public static StorageHelper sh;

      static MemoryStream ReadAndroidFile(string filename) {
         MemoryStream memstream = null;
         if (sh != null &&
             !string.IsNullOrEmpty(filename)) {
            memstream = new MemoryStream();
            using (Stream stream = sh.OpenFile(filename, "r")) {
               byte[] buff = new byte[1024];
               int count;
               while ((count = stream.Read(buff, 0, buff.Length)) > 0) {
                  memstream.Write(buff, 0, count);
               }
      ...
      static void SaveAndroidFile(string filename, Stream stream) {
         if (sh != null &&
             stream != null &&
             !string.IsNullOrEmpty(filename)) {
